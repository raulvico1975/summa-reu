...
export async function GET(
  ...
  const secretKey = await readStripeSecretKeyForOrganization(db, orgId);
  if (!secretKey) {
    return jsonError(
      'STRIPE_NOT_CONFIGURED',
      'Stripe Sync no està configurat per aquesta organització.',
      412
    );
  }
  ...
  try {
    ...
    console.error('[api/stripe/payout] Error inesperat:', error);
    return jsonError(
      'INTERNAL_ERROR',
